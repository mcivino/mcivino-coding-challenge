...
namespace ConstructionLine.CodingChallenge.Tests
{
    ...
    {
        protected static void AssertResults(List<Shirt> shirts, SearchOptions options, List<Shirt> shirtsResult)
        {
            Assert.That(shirts, Is.Not.Null);

            var resultingShirtIds = shirtsResult.Select(s => s.Id).ToList();
            ...
            var expectedCount = shirts.Count(x => sizeIds.Contains(x.Size.Id) && colorIds.Contains(x.Color.Id));

            Assert.AreEqual(expectedCount, shirtsResult.Count);
        }
